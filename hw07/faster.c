...
long long int countContractions( char *text, char *contraction,
                             int textOffset, int contrOffset,
                             int charPerWord, int leftToUse,
                             long long int *cache, int cacheWidth, int cacheHeight )
{
    int cacheIdx = leftToUse * cacheHeight * cacheWidth + contrOffset * cacheWidth + textOffset;
//    printf("%d:", cacheIdx);
    if( cache[ cacheIdx ] )
    {
        if ( cache[ cacheIdx ] == -1 ) return 0;
    ...
    while( *text )
    {
        ...
        if( *text == nextLetter && leftToUse )
        {
            contractionCount += countContractions( text + 1, contraction + 1,
                                                    textOffset + 1, contrOffset + 1,
                                                    charPerWord, leftToUse - 1,
                                                    cache, cacheWidth, cacheHeight );
        }
        text++;
        textOffset++;
    }

    if( contractionCount ) cache[ cacheIdx ] = contractionCount;
    else cache[ cacheIdx ] = -1;
...
int main()
{
    ...
    while ( parseLine( &operation, &inWordChar, &text ) )
    {
        ...
        if( operation == '#' )
        {
            ...
            {
                int cacheSize = ( contractionLength + 4 ) * ( textLength + 4 ) * ( inWordChar + 2 );
                long long int *cache = (long long int *)calloc( cacheSize, sizeof( *cache ));

                long long count = countContractions( text, contraction, 0, 0,
                                                     limit, limit, cache,
                                                     textLength + 2, contractionLength + 2 );
                printf( "> limit %d: %lld\n", limit, count );
                free( cache );
            }
        }
        if( operation == '?' )
        {
            ...
            long long count = findContractions( text, text, contraction, positions, positions,
