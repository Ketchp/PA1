...
int query( int from, int to, int *list, int *changes );
...
int main()
{
    int *visit_count = (int *)malloc( MAX_ID * ( MAX_QUERY / BUCKET_SIZE ) * sizeof(*visit_count) );
...
//                printl(visit_count);
            ...
            to++;
            ret = query( from, to, visit_count, change_list );
            printf( "> %d / %d\n", ret, to - from );
            break;
